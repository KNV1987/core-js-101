...
function getRectangleString(width, height) {
  ...
  const spaceStr = ' '.repeat(width - 2);
  const firstStr = `┌${widthStr}┐\n`;
  const centerStr = `│${spaceStr}│\n`;
  const lasttStr = `└${widthStr}┘\n`;
  return `${firstStr}${centerStr.repeat(height - 2)}${lasttStr}`;
}
